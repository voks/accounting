...
class Subsidiary_account_model extends CI_Model {
	...
	public function sub_account_exist($accountId){
		$sql = "select * from tb_account_subsidiary where sub_code = ?";
	...
	public function sub_account_get($sub_code,$sub_name,$account_type){
		$sql = "select * from tb_account_subsidiary where sub_code like ? and sub_name like ? and account_type = ?";
	...
	public function search_subsidiarymainaccount($account_type,$sub_code,$sub_name){
		$sql = "select * from  tb_account_subsidiary where project_id=".$this->session->userdata('project_id')." and account_type = ? and  sub_code like ? and sub_name like ?";
		return $this->db->query($sql, array($account_type,"%$sub_code%","%$sub_name%"));
	}
