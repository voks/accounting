...
class Subsidiary_account extends CI_Controller {
		...
		if (count($err)) {
			...
			if ($check_id) {
				...
				//foreach ($sub_account_data as $key => $value) {
					 ...
					 $sub_account_data['account_title'] = substr($sub_account_data['account_title'] , 6) ;
				//}
	...
	public function report_tbl(){
		...
		if ($this->session->userdata('islogged')) {
			...
			$data = $this->subsidiary_account_model->search_subsidiarymainaccount($account_type,$account_code,$account_title)->result();
